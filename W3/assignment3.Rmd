---
title: "Spatial Analytics: assignment W3"
output: pdf_document
---
...
crs(cities_sf3035) #verified (starts with '+proj')
...
As the projection is now 'projected', the measurement units are meters rather than degrees. 
...
i_sm <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(str_extract(latitude, "\\d+\\.*\\d*")), #extracting decimal numbers using the stringr library
         longitude = as.numeric(str_extract(longitude, "\\d+\\.*\\d*"))) #extracting decimal numbers using the stringr library
...
From the leaflet, we see that the inscriptions are from Italy. 
...
i <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(str_extract(latitude, "\\d+\\.*\\d*")), #extracting decimal numbers using the stringr library
         longitude = as.numeric(str_extract(longitude, "\\d+\\.*\\d*")))  %>% #extracting decimal numbers using the stringr library 
  filter(!is.na(latitude)) %>% #keeps those that are not NAs
  filter(!is.na(longitude)) %>% #keeps those that are not NAs
...
# Calculating ratio of urban to rural inscriptions
nrow(insc_urban)
nrow(insc_rural)
```
...
By calculating the number of rows of insc_urban and insc_rural, respectively, we see that the ration of rural to urban inscriptions is 10462:1608